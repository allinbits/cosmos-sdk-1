...
func (s *IntegrationTestSuite) TestQueryGrantGRPC() {
	...
	grantee := s.grantee[1]
	grantsURL := val.APIAddress + "/cosmos/authz/v43beta1/grants?granter=%s&grantee=%s&msg_type_url=%s"
	testCases := []struct {
		name      string
		url       string
		expectErr bool
...
func (s *IntegrationTestSuite) TestQueryGrantsGRPC() {
	val := s.network.Validators[0]
	grantee := s.grantee[1]
	grantsURL := val.APIAddress + "/cosmos/authz/v43beta1/grants?granter=%s&grantee=%s"
	testCases := []struct {
		name      string
		url       string
		expectErr bool
