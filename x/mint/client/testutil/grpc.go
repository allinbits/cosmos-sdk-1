...
func (s *IntegrationTestSuite) TestQueryGRPC() {
	...
	testCases := []struct {
		...
		{
			"gRPC request params",
			fmt.Sprintf("%s/cosmos/mint/v42beta1/params", baseURL),
			map[string]string{},
		...
		},
		{
			...
			fmt.Sprintf("%s/cosmos/mint/v42beta1/inflation", baseURL),
			map[string]string{},
		...
		},
		{
			...
			fmt.Sprintf("%s/cosmos/mint/v42beta1/annual_provisions", baseURL),
			map[string]string{
